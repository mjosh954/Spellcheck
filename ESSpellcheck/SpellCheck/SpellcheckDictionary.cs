...
{
    private readonly char[] _alphabetSet;

    public SpellcheckDictionary(IEnumerable<string> words) : base(words)
    {
        _alphabetSet = "abcdefghijklmnopqrstuvwxyz".ToArray();
    }
    ...
    public string GetClosestWord(string word)
    {
        ...
        HashSet<string> candidates = []; // Mutatable
        ...
        return candidates.ElementAt(Random.Shared.Next(candidates.Count)); // why random?
    }
    ...
    private void OneLetterOffWords(string word, HashSet<string> candidates)
    {
        ...
        foreach (char a in _alphabetSet)
        {
